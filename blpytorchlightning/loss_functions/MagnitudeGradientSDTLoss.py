...
class MagnitudeGradientSDTLoss(nn.Module):
    ...
    def forward(self, phi: torch.Tensor) -> torch.Tensor:
        ...
        magnitude_gradient_of_phi = torch.sqrt(
            torch.pow(grads["ddx"], 2) + torch.pow(grads["ddy"], 2) + self.eps
        )
        squared_log_of_magnitude_gradient_of_phi = torch.pow(
            torch.log(magnitude_gradient_of_phi), 2
        )
        phi_not_zero = torch.logical_not(zero_crossings(phi))
        return torch.sum(phi_not_zero * squared_log_of_magnitude_gradient_of_phi) / (
            torch.sum(phi_not_zero) + self.eps
        )


class MagnitudeGradientSDTLoss3D(nn.Module):
    """
    The 3D magnitude gradient (of) Signed Distance Transform loss function.
    """
    def __init__(self, vox_width: float) -> None:
        """
        Initialization method.
        ...
        super().__init__()
        self.eps = 1e-8
        self.kernels = get_3d_first_derivative_kernels()
        self.vox_width = vox_width
    ...
    def forward(self, phi: torch.Tensor) -> torch.Tensor:
        ...
        for k, v in self.kernels.items():
            ...
            grads[k] = torchfunc.conv2d(phi, torch_kernel) / self.vox_width
        magnitude_gradient_of_phi = torch.sqrt(
            torch.pow(grads["ddx"], 2) + torch.pow(grads["ddy"], 2) + torch.pow(grads["ddz"], 2) + self.eps
        )
        squared_log_of_magnitude_gradient_of_phi = torch.pow(
            torch.log(magnitude_gradient_of_phi), 2
        )
